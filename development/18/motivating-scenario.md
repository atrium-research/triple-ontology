...
- `triple:machineTranslatedLanguage` - `owl:DatatypeProperty`, range `xsd:string` (0 or more); each value is the language tag of the title/abstract versions produced by machine translation. The plain language of each version remains the language tag of the literal; the provider's raw language value is `triple:originalInLanguage` (iteration 17). This assumes title and abstract share their translation status per language; should per-string granularity ever be needed, a text-version reification would replace this property.
