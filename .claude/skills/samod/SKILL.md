...
| Current Model | TBox released at end of iteration N-1                                | `ontology/triple.ttl` (+ `docs/*`)               |
...
| Merge (Phase 2)                | `scripts/merge_iterations.py` — the default output is the canonical `ontology/triple.ttl`. |
| Milestones                     | Git commits at the end of each SAMOD phase (`samod: iter NN phase 1 done` style)                |
| Documentation output           | The published surface is `docs/`: the landing index plus one page per artefact (`triple/` and the six vocabularies). The model page is chaptered like the SKOS Reference: narrative chapters in `ontology/doc/sections/`, Graffoo figures in `ontology/doc/figures/` (embedded in the chapters), guidance in RDF (`skos:scopeNote`/`skos:example` at the term's home). Full spec: `references/documentation.md`. |
...
3. Run merge:
   ...
   ```
   The default output is the canonical `ontology/triple.ttl`.
