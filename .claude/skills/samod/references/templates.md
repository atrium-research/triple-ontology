...
**Hard rule — TBox only.** Every row must be a name that appears in `TBOX.ttl`. Do not list ABox individuals. Do not list `prtype:network` or `ct:article` and so on unless the concept scheme itself is introduced in this iteration's TBox.
