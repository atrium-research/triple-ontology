...
|---|---|
| `python merge_iterations.py` | Merges every `development/NN/TBOX.ttl` plus `ontology/metadata.ttl` into the consolidated model. The default output **is** the canonical `ontology/triple.ttl` (the legacy `--output` ritual is gone). |
