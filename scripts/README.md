...
|---|---|
| `python merge_iterations.py` | Merges every `development/NN/TBOX.ttl` plus `docs/triple/metadata.ttl` into the consolidated model. The default output **is** the canonical `docs/triple/triple.ttl` (the legacy `--output` ritual is gone). |
...
| `scripts/build_docs.sh [output-base]` | Regenerates **the whole published surface**: the seven documentation pages — the model (SKOS-Reference-style chapters from `docs/triple/doc/sections/`, figures from `docs/triple/doc/figures/`) plus the six vocabularies — with the vendored pyLODE fork in `tools/pylode/`, and the landing `index.html` at the root (one card per artefact, via `build_index.py`). Recompiles the vocabularies (`build.py`) first; every page ships with its `static/` and `.ttl`/`.rdf`/`.jsonld` serializations. No argument = preview in `build/docs-preview/` (git-ignored); `docs` = the official pages. |
| `python scripts/build_index.py -o <file>` | The landing page alone — one card per artefact (titles, descriptions, versions, term counts read from `docs/triple/triple.ttl` and the compiled vocabularies in `build/`, so it can never go stale). Normally driven by `build_docs.sh`. |
